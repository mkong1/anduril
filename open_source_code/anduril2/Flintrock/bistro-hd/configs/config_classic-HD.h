 ...
 *
 * Bistro configuration. 2017, (C) Flintrock (FR).
...
/*******Soft Start***********/
#define SOFT_START  // soft transitions between modes.

// Options for first bootup/default:

#define USE_FIRSTBOOT //Enables reset menu option, only costs a couple of bytes.

#define INIT_MODEGROUP      5       // which mode group will be default, mode groups below start at zero, select the mode group you want and subtract one from the number to get it by defualt here
...
#define INIT_MAXTEMP       79       // maximum temperature
